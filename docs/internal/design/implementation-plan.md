...
*Execution-ready roadmap for Skyhook HPCaaS on EKS/SkyPilot*

Ordered steps to move from architecture to running platform. Each step is scoped to produce a verifiable artifact (manifest, policy, script, or test).
...
| Multi-node with EFA | NCCL all-reduce latency/throughput; Kyverno env injection |
...
```bash
# Manifests dry run (no cluster access required)
make dry-run-manifests

# FSx mount check (on node)
mount | grep lustre && dd if=/dev/zero of=/mnt/fsx/test bs=1M count=1024

# NCCL/EFA quick check (in 2-node job)
nccl-tests/all_reduce_perf -b 8 -e 1G -f 2 -g 8
# Confirm transport "efa" in logs

# SOCI cold-start timing
# Schedule pod with large image, measure ContainerCreating duration
# Compare with and without SOCI index

# Spot simulation (if allowed)
aws ec2 terminate-instances --instance-ids <id>
# Watch checkpoint + reschedule
```
...
| Artifact | Location |
|----------|----------|
| Karpenter EC2NodeClass and NodePool manifests | `skypilot-infra/karpenter-hpc.yaml` |
| Launch Templates (placement groups, EFA ENIs) | CloudFormation/Terraform |
| NVMe RAID0 + FSx mount bootstrap script | User data / systemd unit |
| Kyverno policies | `skypilot-infra/kyverno-*.yaml` |
| SOCI enablement config | containerd/Bottlerocket config |
| Observability manifests | `platform/base/observability/`, `platform/base/logging/` |
